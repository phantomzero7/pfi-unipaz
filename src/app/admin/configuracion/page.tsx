    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <section className="rounded-3xl bg-white dark:bg-slate-900/60 backdrop-blur-xl border border-slate-200/90 dark:border-white/10 p-6 sm:p-8 shadow-sm dark:shadow-xl space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200 dark:border-white/10 pb-4">
          <div>
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-blue-600 bg-blue-100 dark:bg-blue-500/20 px-2.5 py-0.5 rounded-full">
                Calendario Institucional
              </span>
            </div>
            <h2 className="text-lg font-black text-unipaz-navy dark:text-white tracking-tight flex items-center gap-2 mt-1">
              <Calendar className="w-5 h-5 text-blue-600" />
              2. Periodos Cuatrimestrales y Semestrales por Programa Académico
            </h2>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Configura los periodos oficiales vigentes (ej. <strong>Cuatrimestral 187 Mayo-Agosto 2026</strong> o <strong>Semestral 902 Febrero-Julio 2026 para Médico Cirujano</strong>). Las actividades y puntos se ajustan al rango de fechas configurado.
            </p>
          </div>

          {periodMsg && (
            <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
              <CheckCircle2 className="w-4 h-4" /> {periodMsg}
            </span>
          )}
        </div>

        {/* Formulario para Crear Nuevo Periodo */}
        <form onSubmit={handleAddPeriod} className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-white/10 space-y-4">
          <div className="flex items-center gap-2">
            <Plus className="w-4 h-4 text-blue-600" />
            <h4 className="text-xs font-black text-unipaz-navy dark:text-white">
              Dar de Alta Nuevo Periodo Académico
            </h4>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 text-xs">
            <div>
              ...
              <input
                type="text"
                required
                placeholder="ej. 187, 902, 188..."
                value={newPeriodCodigo}
                onChange={(e) => setNewPeriodCodigo(e.target.value)}
                className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-white/15 rounded-xl p-2 font-mono font-bold text-xs"
              />
            </div>
            ...
            <div className="lg:col-span-2">
              ...
              <input
                ...
                className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-white/15 rounded-xl p-2 text-xs font-semibold"
              />
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-white/15 rounded-xl p-2 text-xs font-bold"
              >
                <option value="cuatrimestral">Cuatrimestral (Licenciaturas y Maestrías)</option>
            ...
            <div className="flex items-end">
              <button
                type="submit"
                className="w-full py-2.5 px-4 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-black text-xs flex items-center justify-center gap-1.5 shadow-sm transition-all hover:scale-102"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs pt-1 border-t border-slate-200 dark:border-white/10">
            <div>
              <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="Observaciones de ciclo..."
                value={newPeriodDesc}
          ...
          <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-white/10 space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="font-black text-xs text-unipaz-navy dark:text-white uppercase tracking-wider flex items-center gap-1.5">
                ...
                Periodos Cuatrimestrales (Licenciaturas & Maestrías)
              </h4>
            </div>

            <div className="space-y-2">
              {(pfiConfig.periodosAcademicos || [])
                .filter((p) => p.tipo === 'cuatrimestral')
                .map((period) => (
                  <div
                    key={period.id}
                    className={`p-3.5 rounded-xl border transition-all ${
                      period.es_actual
                        ? 'bg-orange-50/80 dark:bg-orange-950/30 border-unipaz-orange shadow-sm'
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-mono font-black text-xs text-unipaz-navy dark:text-white bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md">
                          ...
                          {period.es_actual && (
                            <span className="px-2 py-0.5 rounded-full bg-unipaz-orange text-slate-950 font-black text-[10px]">
                              ACTIVO VIGENTE
                            </span>
                          )}
                        </div>
                        <div className="text-[11px] font-mono text-slate-500 mt-1">
                          Vigencia: {period.fecha_inicio} al {period.fecha_fin}
                        </div>
                        {period.descripcion && (
                          <div className="text-[10px] text-slate-400 mt-0.5">{period.descripcion}</div>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        {!period.es_actual && (
                          <button
                            type="button"
                            onClick={() => setCurrentAcademicPeriod(period.id, 'cuatrimestral')}
                            className="py-1 px-2.5 rounded-lg bg-slate-100 hover:bg-unipaz-orange hover:text-slate-950 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold text-[10px] transition-all"
                          >
                            Hacer Vigente
                          </button>
                        )}
                        <button
                          type="button"
                          onClick={() => deleteAcademicPeriod(period.id)}
          ...
          <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-white/10 space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="font-black text-xs text-unipaz-navy dark:text-white uppercase tracking-wider flex items-center gap-1.5">
                ...
                Periodos Semestrales (Médico Cirujano)
              </h4>
            </div>

            <div className="space-y-2">
              {(pfiConfig.periodosAcademicos || [])
                .filter((p) => p.tipo === 'semestral')
                .map((period) => (
                  <div
                    key={period.id}
                    className={`p-3.5 rounded-xl border transition-all ${
                    ...
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        {!period.es_actual && (
                          <button
                            type="button"
                            onClick={() => setCurrentAcademicPeriod(period.id, 'semestral')}
                            className="py-1 px-2.5 rounded-lg bg-slate-100 hover:bg-blue-600 hover:text-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold text-[10px] transition-all"
                          >
                            Hacer Vigente
                          </button>
                        )}
