    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="bg-slate-900/60 backdrop-blur-xl border border-white/10 p-6 sm:p-8 rounded-3xl shadow-xl">
        ...
        <div className="mt-6 relative max-w-xl">
          ...
          <input
            ...
            className="w-full bg-slate-950/80 border border-white/15 rounded-2xl pl-11 pr-4 py-3 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-unipaz-orange"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredStudents.map((std) => {
          ...
          return (
            <div
              key={std.id}
              className="rounded-3xl bg-slate-900/60 backdrop-blur-xl border border-white/10 p-6 shadow-xl space-y-4 hover:border-unipaz-orange/40 transition-all flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center gap-3">
                  <div className="relative w-12 h-12 rounded-2xl overflow-hidden border border-white/20 flex-shrink-0">
                    <Image
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">{std.carrera}</p>
                ...
                {/* Métricas de Horas */}
                <div className="mt-4 p-3 rounded-2xl bg-slate-950/70 border border-white/10 space-y-2">
                  ...
                  <div className="flex items-center justify-between text-[11px] pt-1">
                    ...
                    <span
                      className={`font-bold ${
                        prog.isAcreditado ? 'text-emerald-400' : 'text-amber-400'
                      }`}
                    >
              ...
              <button
                ...
                className="w-full py-2.5 px-4 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white text-xs font-bold flex items-center justify-center gap-1.5 transition-colors"
              >
      ...
      {selectedStudent && selectedStudentProgress && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md animate-fadeIn overflow-y-auto">
          <div className="relative w-full max-w-3xl bg-slate-900 border border-white/15 rounded-3xl p-6 sm:p-8 shadow-2xl text-white my-8 space-y-6">
            <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
              ...
              <div className="p-3.5 rounded-2xl bg-slate-950 border border-white/10 space-y-1">
                <span className="text-slate-400">3 Talleres Extracurr.:</span>
                ...
                <div className={`text-[10px] ${selectedStudentProgress.talleresExtracurriculares.cumplido ? 'text-emerald-400 font-bold' : 'text-amber-400'}`}>
                  {selectedStudentProgress.talleresExtracurriculares.cumplido ? '✓ Cumplido' : 'Pendiente'}
                </div>
              </div>

              <div className="p-3.5 rounded-2xl bg-slate-950 border border-white/10 space-y-1">
                <span className="text-slate-400">Plan de Vida y Carrera:</span>
                <div className="font-bold text-white">
                  {selectedStudentProgress.pvc.horas.toFixed(1)} / 75.00 hrs
                </div>
                <div className={`text-[10px] ${selectedStudentProgress.pvc.cumplido ? 'text-emerald-400 font-bold' : 'text-amber-400'}`}>
            ...
            <div className="space-y-2">
              ...
              <div className="max-h-60 overflow-y-auto rounded-2xl bg-slate-950 border border-white/10">
                <table className="w-full text-left text-xs">
                  <thead>
                    <tr className="border-b border-white/10 text-slate-400 bg-slate-900/80 sticky top-0">
                    ...
                    {selectedStudentAttendances.map((att) => (
                      <tr key={att.id} className="hover:bg-slate-800/40">
                        ...
                        <td className="py-2.5 px-3 capitalize">
                          {att.status === 'asistio' ? (
                            <span className="text-emerald-400 font-bold">Acreditado</span>
                          ) : (
                            <span className="text-amber-400">{att.status}</span>
                          )}
                        </td>
                        ...
                        <td className="py-2.5 px-3 text-right">
                          {att.status !== 'asistio' && (
                            <button
                              ...
                              className="px-2.5 py-1 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-bold text-[10px]"
                            >
                              Aprobar
                            </button>
                          )}
            ...
            <div className="pt-2 flex justify-end">
              <button
                onClick={() => setSelectedStudent(null)}
                className="py-2.5 px-6 rounded-xl bg-slate-800 text-slate-200 hover:text-white font-bold text-xs"
              >
                Cerrar Expediente
              </button>
            </div>
