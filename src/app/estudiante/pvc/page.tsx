  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="bg-slate-900/60 backdrop-blur-xl border border-white/10 p-6 sm:p-8 rounded-3xl shadow-xl">
        ...
        <div className="mt-6 p-4 rounded-2xl bg-slate-950/80 border border-white/10">
          <div className="flex items-center justify-between text-xs font-bold mb-2">
            <span className="text-slate-300 flex items-center gap-1.5">
              <Compass className="w-4 h-4 text-purple-400" />
              Avance del Bloque PVC:
            </span>
            <span className={progress.pvc.cumplido ? 'text-emerald-400' : 'text-amber-400'}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {pvcModules.map((m, index) => (
          <div
            key={m.id}
            className={`p-6 rounded-3xl border transition-all relative overflow-hidden ${
              m.completed
                ? 'bg-slate-900/80 border-emerald-500/40 shadow-xl shadow-emerald-500/5'
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <span className="text-xs px-2.5 py-0.5 rounded-full bg-purple-500/20 text-purple-300 border border-purple-400/30 font-bold">
                {m.tag}
              </span>
            ...
            <h3 className="text-lg font-black text-white mt-4">{m.title}</h3>
            <p className="text-xs text-slate-400 mt-2 leading-relaxed">{m.description}</p>

            <div className="mt-6 pt-4 border-t border-white/10 flex items-center justify-between text-xs">
              <span className="text-slate-400">Valor Curricular:</span>
              <span className="font-mono font-bold text-white">+{m.hours.toFixed(2)} hrs</span>
            </div>
          </div>
        ))}
      </div>
